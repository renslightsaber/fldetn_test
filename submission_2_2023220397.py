...
FLDETN_DIR = MODELS_DIR / "FLDetn"

sys.path.insert(0, str(MODELS_DIR))    # for: from models import FLDetn
sys.path.insert(0, str(FLDETN_DIR))    # for: from ultralytics...

# 3. 이제 import!
import safe_utils
from safe_utils import YOLO_safe, setup_safe_globals

from models import FLDetn
# from ultralytics import YOLO

# monkey patch 확인
# from ultralytics.models.yolo.model import YOLO as YOLO_CLASS
# from ultralytics.engine.model import Model as EngineModel

# print("✅ YOLO.train =", YOLO_CLASS.train)
# print("✅ EngineModel.train =", EngineModel.train)


def submission_2_2023220397(yaml_path, output_json_path):
    ...
    ex_dict['Device'] = str(ex_dict['Device'])
    ...
    # model = YOLO(f'models/FLDetn/{model_name}.yaml', verbose=False)
    if model_name == "FLDet-n":
        model_yaml_path = f'models/FLDetn/{model_name}.yaml'
        model = YOLO_safe(model_yaml_path)

    os.makedirs(output_dir, exist_ok=True)
    ex_dict['Model Name'] = model_name; ex_dict['Model']=model; 
    ex_dict = FLDetn.train_model(ex_dict, model, model_yaml_path) ### Check
